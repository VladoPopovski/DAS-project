...
class WebDriverSingleton:
    """
    Singleton class for managing WebDriver instance.
    """
    ...
    def get_instance():
        ...
        if WebDriverSingleton._instance is None:
            WebDriverSingleton._instance = webdriver.Chrome(options=WebDriverSingleton._get_chrome_options())
    ...
    def _get_chrome_options():
        ...
        """
        download_dir = os.path.join(os.path.expanduser("~"), "Desktop", "MSEData")
...
class DataDownloader:
    """
    Class to download and rename issuer data.
    """

    def __init__(self):
        self.driver = WebDriverSingleton.get_instance()
        self.base_url = "https://www.mse.mk/en/stats/symbolhistory/kmb"
        self.download_dir = os.path.join(os.path.expanduser("~"), "Desktop", "MSEData")
    ...
    def get_issuers(self):
        ...
        issuer_dropdown = self.driver.find_element(By.ID, "Code")
        ...
        with open(os.path.join(self.download_dir, "issuers.json"), "w") as f:
            json.dump(issuers, f, indent=4)

        return issuers

    def wait_for_all_downloads(self, timeout=30):
        """
    ...
    def rename_downloaded_file(self, issuer_name, start_date, end_date):
        """
        ...
        if original_file:
            ...
            original_file_path = os.path.join(self.download_dir, original_file)
            os.rename(original_file_path, new_file_path)
            print(f"File renamed to: {new_file_name}")

    def download_issuer_data(self, issuer_code, issuer_name):
        """
        ...
        for year in range(years_to_iterate):
            ...
            find_button = self.driver.find_element(By.CSS_SELECTOR,
                                                   "#report-filter-container > ul > li.container-end > input")
            find_button.click()

            try:
                rows = self.driver.find_elements(By.CSS_SELECTOR, "#resultsTable tbody tr")
                if not rows:
            ...
            download_button = self.driver.find_element(By.CSS_SELECTOR, "#btnExport > i")
            download_button.click()

            if self.wait_for_all_downloads():
                self.rename_downloaded_file(issuer_name, start_date, end_date)
    ...
    def main(self):
        ...
        for issuer_code, issuer_name in issuers:
            ...
            try:
                self.download_issuer_data(issuer_code, issuer_name)
